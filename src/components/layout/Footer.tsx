...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-border-subtle bg-bg-primary py-8">
      <div className="max-w-content mx-auto px-6 md:px-20 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-caption text-text-secondary normal-case tracking-normal">
          Harkeerat Bhasin · © {year} · Built with care.
        </p>
        <div className="flex items-center gap-4">
          {SOCIAL.map((s) => (
            <a
              ...
              aria-label={s.label}
              className={`text-text-secondary hover:text-accent-electric transition-colors duration-150 ${FOCUS_RING}`}
            >
              {s.icon}
            </a>
          ))}
